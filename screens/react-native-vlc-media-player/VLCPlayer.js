...
export default class VLCPlayer extends Component {
  ...
  audioTrack(id) {
    this.setNativeProps({ audioTrack: id });
  }
  ...
  _onPaused(event) {
    if (this.props.onPaused) {
      ...
      // this.setNativeProps({ audioTrack: 2 });
    }
  ...
  _onLoad(event) {
    ...
    if (this.props.onLoad) {
      ...
      // this.setNativeProps({ audioTrack: 1 });
      const { audioTrack } = this.props;
      const { textTrack } = this.props;
      const { rate } = this.props;
      const { videoAspectRatio } = this.props;
  
      // Use the received prop value to set the new audioTrack
      this.setNativeProps({
        audioTrack,textTrack,rate, videoAspectRatio
        // Add other props you want to update here
      });
  ...
  render() {
    ...
    source.initOptions = source.initOptions || ['--adaptive-logic=lowest', '--brightness=2.0'
  ];
