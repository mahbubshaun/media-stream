...
export default class ControlBtn extends Component {
  ...
  handleSlidingComplete = (value) => {
    ...
    const newValue = minimumValue + touchPercent * (maximumValue - minimumValue);
...
<View style={{  }}>
  <Text style={{ fontSize: 15, color: '#fff', marginRight: 10 }}>
...
<View>
{
 ...
 <Icon name={'subtitles'} size={30} color="#fff" style={{ marginRight: 10 }} onPress={() => this.bottomSheetRef.current.open()}/>
       
    <Icon name={paused ? 'play-circle-outline' : 'pause-circle-outline'} onPress={() => {
                      onPausedPress && onPausedPress(!paused);
                    }} size={40} color="#fff" style={{ marginRight: '0%' }}/>

    <Icon name={isFull ? 'fullscreen-exit' : 'fullscreen'} size={30} color="#fff"  onPress={() => {
...
<RBSheet
        ...
        style={{ color: 'white', width: '100%', textAlign: 'center', marginTop: 20}} // Set text color and center the text for the Picker
      ...
      <Picker.Item key={index} label={track.name} value={track.id} />
    ))}
