...
import { Text, View, Image, StatusBar, ScrollView, TouchableOpacity,  FlatList, StyleSheet, Dimensions } from 'react-native';
...
// console.log("Media Link:", mediaLink);
    ...
    const [inRatingList, setRatingList] = useState(
        ratinglist && ratinglist.hasOwnProperty(details.id) ? ratinglist[details.id]?.rating || 0: 0
      ); 
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const trailer = await axios.get(`${config.API_URL}/movie/${details.id}/videos?language=en-US`, {headers});
                const trailers = trailer.data.results;

                // Filter out only the trailers
               const trailerResults = trailers.filter(trailer => trailer.type === 'Trailer');
            ...
            try {
                const suggested = await axios.get(`${config.API_URL}/movie/${details.id}/recommendations?language=en-US&page=1`, {headers});
                setSuggested(suggested.data.results);
            } catch (error) {
                console.error('Error fetching suggested movies:', error);
            }
            const castName = await axios.get(`${config.API_URL}/movie/${details.id}/credits?language=en-US`, {headers});
            console.log('cast details');
            console.log(castName.data);
            setCast(castName.data.cast);

            try {
                const revieData = await axios.get(`${config.API_URL}/movie/${details.id}/reviews?language=en-US&page=1`, {headers});
    ...
    const getCast = async (tmdbId, category) => {
        ...
        }/${tmdbId}/credits?language=en-US`, {headers})
       ...
       <ScrollView style={style.main} key={details}>
           ...
           <View style={[style.flexboxContainer, style.flexColumn, {paddingBottom: 30}]}>
               <View style={[movieDetails.moviePoster, style.relative]}>
                   ...
                   <FastImage source={{uri: `https://image.tmdb.org/t/p/original${details.backdrop_path}`, priority: FastImage.priority.high}} style={[movieDetails.moviePoster]} resizeMode="cover"/>
                   <LinearGradient colors={['rgba(49,54,74,0.2)', 'rgba(49,54,74,0.4)', '#31364A']} angle={180} style={[movieDetails.gradiend, style.flexboxContainer, style.centerAligned, style.flexColumn]}>
                       {trailer != '' && 
                           <TouchableOpacity style={[style.centerAligned]} onPress={() => {navigation.push('Trailer', {data: trailer})}}>
               ...
               <View style={[movieDetails.detailsContainer, style.relative]}>
                   <View style={[style.flexboxContainer, style.flexStart]}>
                       ...
                       <FastImage source={{uri: `https://image.tmdb.org/t/p/w500${details.poster_path}`, priority: FastImage.priority.high}} style={[movieDetails.selectedMoviePoster]} resizeMode="cover"/>
                       </View>
                       <View style={[style.flexbox]}>
                           ...
                           <Text style={[movieDetails.date, movieDetails.leftWidth]} numberOfLines={1} ellipsizeMode="tail">Release Date: {details.release_date}</Text>
                           <View style={[style.con]}>
                           <Text style={[movie.subText, style.border, movie.posterGenre, { marginTop: 10, height: 25, fontWeight: "700"}]} >{details.vote_average.toFixed(1)}</Text>
...
/>
                   ...
                   <View >
                       <Text style={[style.textCenter, {color: 'white', fontWeight: "800"}]}>
                           Rate this movie
                       </Text>
                       <AirbnbRating
 count={11}
 reviews={["Terrible", "Bad", "Meh", "OK", "Good", "Hmm...", "Very Good", "Wow", "Amazing", "Unbelievable", "Mind bending"]}
...
/>
                   ...
                   <View style={{paddingTop: 15}}>
                       <Text style={movieDetails.description}>{details.overview}</Text>
                   </View>
                  {savedTitle === details.title && (
                   <TouchableOpacity style={{marginTop: 35}} onPress={handleStream}>
                       <LinearGradient colors={['#FE6FA1', '#FF6EA0', '#FD70AF']} angle={180} style={style.btn}> 
                           <Text style={[style.btnText, style.textCenter, style.textUppercase]}>Stream Now!</Text>
                       </LinearGradient>
                   </TouchableOpacity>
                    )} 
               </View>
